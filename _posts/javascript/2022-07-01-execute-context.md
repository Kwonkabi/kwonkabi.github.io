...
- **1. 전역 코드**
  ...
  - 이를 위해 전역 코드가 평가되면 **전역 실행 컨텍스트**가 생성됨 <br>
...
- **2. 함수 코드**
  ...
  - 이를 위해 함수 코드가 평가되면 **함수 실행 컨텍스트**가 생성됨 <br>

- **3. eval 코드**

  - 빌트인 전역함수인 eval 함수에 인수로 전달되어 실행되는 소스코드
  - strict mode에서 자신만의 독자적인 스코프를 생성함
  - => eval 코드가 실행되면 **eval 실행 컨텍스트**가 생성됨 <br>
...
![](/assets/images/js/execute_context.jpeg)
